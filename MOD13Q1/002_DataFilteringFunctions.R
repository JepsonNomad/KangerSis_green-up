...
# 2. Lower 0.025 quartile NDVI for each pixel time series is used as winter NDVI
...
winterNDVI = function(x, lpy = 23){
  ...
  x[x==0] <- NA # Remove -99999 values (GEE Mask flag)
...
# x = as.vector(NDVI.winter[1,23,])
# plot(ts_scale(x),type="l")
#### Curve fitting ----
...
library(phenex)
library(DEoptim)
yearphenology = function(x, yearLayers, yearIndex,
                         lpy = 23, approach = "phenex"){
  # Extract NDVI and day of year values
  x.NDVI = x[,1]
  y.DOY = x[,2]

  if(approach == "phenex"){
    if(sum(x.NDVI > 0, na.rm=T) >= 5){
      x.phenex = rep(NA, length.out = 366)
      x.phenex[y.DOY[!is.na(y.DOY)]] = x.NDVI[!is.na(y.DOY)]
      # plot(x.phenex)
      x.modeled = phenex::modelNDVI(ndvi.values = x.phenex,
                                    year.int = 2000 + yearIndex,
                                    method = "DLogistic",
                                    silent = FALSE)
      x.modelVals = x.modeled[[1]]@modelledValues
      # message("NDVI time series modeled")
      springDate = phenex::phenoPhase(x.modeled[[1]], 
                                      phase = "greenup", threshold = 0.5, 
                                      method = "local")
      fallDate = phenex::phenoPhase(x.modeled[[1]], 
                                    phase = "senescence", threshold = 0.5, 
                                    method = "local")
      # message("Phenology dates calculated")
      return(c("springMean" = springDate$mean, "springSD" = springDate$sd, 
               "fallMean" = fallDate$mean, "fallSD" = fallDate$sd))
    }else(return(c("springMean" = NA, "springSD" = NA, "fallMean" = NA, "fallSD" = NA)))
  }
  if(approach == "DE"){
    if(sum(x.NDVI > 0, na.rm=T) >= 1 && sum(abs(diff(x.NDVI)), na.rm=T) > 0){
      ...
      ndviPheno = c("xmidS" = xmidS, "scalS" = scalS, 
                    "xmidA" = xmidA, "scalA" = scalA)
      
      # Return two-layer array with melt season and snowfall onset
      return(ndviPheno)
    } else{return(c("xmidS" = NA, "scalS" = NA, "xmidA" = NA, "scalA" = NA))}
  }
}
...
#### Declare parameters and test the functions ----
# x = c(-10,-5,0,-1,0,
#       10,20,100,130,132,
#       125,140,120,110,130,
#       90,20,10,0,-14,
#       -11,-14,-1) # Sample NDVI series
# 
# y = seq(from=1,to=366,by=16) # Sample DOY series

#### Run functions ----
# plot(x, type = "l")
# 
# # Correct for winter values (step 2)
# wx = winterNDVI(x) 
# lines(wx, type = "l", col="red")
# 
# # Apply moving median window
# mx = medianwindow(wx) 
# lines(mx, type = "l", col="blue")
# 
# # Scale the timeseries to (0,1)
# sx = ts_scale(mx) 
# sx
# plot(sx, type = "l")
# 
# # Fit nls curve
# phenology = yearphenology(x = sx,
#                           y = y)
